...
> Anthropic 官方 17 個精選 skill（docx/pdf/pptx/xlsx、mcp-builder、skill-creator、canvas-design、brand-guidelines、frontend-design、web-artifacts-builder、webapp-testing、theme-factory、algorithmic-art、internal-comms、doc-coauthoring、slack-gif-creator、claude-api-ref）**已經 vendor 進本 repo**（其中 `claude-api` 因與 Claude Code 內建同名，改名為 `claude-api-ref` 並設為被動觸發），直接可用，不在此索引內。
